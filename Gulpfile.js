...
function bundle(opts) {
  ...
  var bundler = browserify(config.scripts.entrypoint,{
      debug: true,
      paths: config.scripts.includePaths,
      cache: {},
      packageCache: {}
    })
    .transform(babelify, {
...
gulp.task("serve", function () {
  var app = express();
  app.use(express.static(path.join(__dirname, config.serve)));
...
gulp.task("watch", function () {
  ...
  gulp.watch(config.watch, ["jekyll:build"], function (file) {
    livereload.changed(file.path);
  });
});

gulp.task("jekyll:build", function(callback){
  exec('jekyll build -Iq', function (err) {
    callback(err);
  });
});
